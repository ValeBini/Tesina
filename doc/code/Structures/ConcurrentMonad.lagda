...
record ConcurrentMonad (M : Set → Set) : Set₁ where
  ...
  field
    ...
    bcomp≲ₘ   : ∀ {A B : Set} → (a₁ a₂ : M A) → (f₁ f₂ : A → M B) → a₁ ≲ₘ a₂ 
                → (∀ (a : A) → (f₁ a) ≲ₘ (f₂ a)) → (a₁ ≫= f₁) ≲ₘ (a₂ ≫= f₂) 
  ...
  field
    ...
    idr       : ∀ {A : Set} → (a : M A) 
                  → (merge a unit) ≅ₘ (a ≫= (λ a → return (a , tt)))
    idl       : ∀ {B : Set} → (b : M B) 
                  → (merge unit b) ≅ₘ (b ≫= (λ b → return (tt , b)))
    assoc     : ∀ {A B C : Set} → (a : M A) (b : M B) (c : M C) 
                  → ((merge (merge a b) c) ≫= (λ {((a , b) , c) → return (a , (b , c))})) 
                                                                ≅ₘ (merge a (merge b c))
    comm      : ∀ {A B : Set} → (a : M A) (b : M B) 
                  → (merge a b) ≅ₘ ((merge b a) ≫= (λ {(a , b) → return (b , a)}))
