...
\AgdaFunction{sym-sum} y \AgdaFunction{sym-max} prueban que ambas operaciones son conmutativas (para la desigualdad):
\ExecuteMetaData[latex/ConaturalsM.tex]{syms}

\AgdaFunction{$\gtrsim$sum} prueba que la suma es compatible con la relación de orden, para ello se prueba también \AgdaFunction{$\gtrsim$sumzero} que prueba lo mismo para el caso en el que el argumento \AgdaBound{n$_2$} es \AgdaInductiveConstructor{zero}:
