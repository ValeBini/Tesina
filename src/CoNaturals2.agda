...
sum-comm : {m n : Coℕ} → sum m n ≳ sum n m
sum-comm {zero}  {zero}  = zero
sum-comm {zero}  {suc n} = refl≳
sum-comm {suc m} {zero}  = refl≳
sum-comm {suc m} {suc n} = suc (♯ suc (♯ (sum-comm {♭ m} {♭ n})))

max-comm : {m n : Coℕ} → max m n ≳ max n m 
max-comm {zero}  {zero}  = zero
max-comm {zero}  {suc n} = refl≳
max-comm {suc m} {zero}  = refl≳
max-comm {suc m} {suc n} = suc (♯ max-comm {♭ m} {♭ n}) 
...
≳sumzero {.(suc _)} {.(suc _)} {.(suc _)} (suc x)  (sucˡ q) = suc (♯ (sucˡ (♯ (trans≳ (≳sumzero (♭ x) ≳zero) sumzero₂))))
≳sumzero {.(suc _)} {m₂}       {.zero}    (sucˡ p) zero     = sucˡ (♯ (trans≳ (♭ p) sumzero₁))
≳sumzero {.(suc _)} {m₂}       {.(suc _)} (sucˡ p) (sucˡ q) = sucˡ (♯ (sucˡ (♯ (≳sumzero (♭ p) ≳zero))))
...
≳sum {suc m}    {zero}     {suc n}    {.(suc _)} (sucˡ ≳m) (suc x)   = suc (♯ (sucˡ (trans≳ (trans≳ (sum-comm {♭ m} {♭ n}) (≳sumzero (♭ x) ≳m)) sumzero₂))) -- suc (♯ (sucˡ (trans≳ (≳sum refl≳ (♭ x)) ≳sum₂)))
...
≳max {suc m}    {zero}     {suc n}    {.(suc _)} (sucˡ p) (suc x)  = suc (♯ trans≳ (trans≳ (max-comm {♭ m} {♭ n}) (≳maxzero (♭ x) p)) maxzero₂)
...
...                              | zero   | [ eq ] = suc (♯ (trans≳ (suc (♯ (≳max (≡⇒≳ eq) (refl≳ {♭ d})))) (trans≳ (trans≳ maxzero₁ (max-comm {suc d} {zero})) (≳max (≡⇒≳ (sym eq)) (refl≳ {suc d})))))
...
...                              | suc c₁ | [ eq ] = suc (♯ (trans≳ (suc (♯ (sucˡ (trans≳ (sum-comm {♭ b} {♭ c}) (trans≳ (≳sum (trans≳ (≡⇒≳ eq) ≳suc) refl≳) (sum≳max {♭ c₁} {♭ b})))))) ((≳max (≡⇒≳ (sym eq)) (refl≳ {suc b}))))) 
...
interchange zero    (suc b) (suc c) zero    = suc (♯ sucˡ (trans≳ (sum-comm {♭ b} {♭ c}) (sum≳max {♭ c} {♭ b})))
